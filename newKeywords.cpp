...
public:
    Player() : m_healthy(100), m_level(1), m_attack(10){};
    Player(int health, int level, int attack)
    {
        m_healthy = health;
        m_level = level;
        m_attack = attack;
...
int main()
{
    Player player1 = string("xgao"); // 实际上进行了隐式转换 Player player1 = Player(string("xgao");
